...
TSharedRef<FImGuiTextInputMethodContext> FImGuiTextInputMethodContext::Create(const TSharedRef<SImGuiWidget>& Widget)
{
...
bool FImGuiTextInputMethodContext::GetTextBounds(const uint32 BeginIndex, const uint32 Length, FVector2D& Position,	FVector2D& Size)
{
	if (OwnerWidget.IsValid())
	{
		...
		if (ImGuiContext)
		{
      const auto MousePos = OwnerWidget.Pin()->GetMousePosition();
      Position = FVector2D(
          CachedGeometry.AbsolutePosition.X + MousePos.X,
          CachedGeometry.AbsolutePosition.Y + MousePos.Y + 20);
		}
	}
	return false;
}
...
FImGuiTextInputMethodContext::FImGuiTextInputMethodContext(const TSharedRef<SImGuiWidget>& Widget)
	: OwnerWidget(Widget)
	, bIsComposing(false)
	, CompositionBeginIndex(0)
	, CompositionLength(0)
