...
FImGuiTextureHandle FImGuiModule::FindTextureHandle(const FName& Name)
{
	const TextureIndex Index = ImGuiModuleManager->GetTextureManager().FindTextureIndex(Name);
        return (Index != INDEX_NONE)
                   ? FImGuiTextureHandle{Name,
                                         ImGuiInterops::ToImTextureID(Index)}
                   : FImGuiTextureHandle{FName(TEXT("ImGuiModule_Empty")),
                                         ImGuiInterops::ToImTextureID(ImGuiModuleManager->GetTextureManager()
                                             .FindTextureIndex(FName(
                                                 TEXT("ImGuiModule_Empty"))))};
}
...
void FImGuiModule::ClearTextures() {
  ImGuiModuleManager->GetTextureManager().ClearTextureResources();
  ImGuiModuleManager->bTexturesLoaded = false;
}
...
void FImGuiModule::NotifyActiveImGuiInputText(ImGuiInputTextCallbackData* Data)
{
  if (ImGuiModuleManager) {
    ImGuiModuleManager->NotifyActiveImGuiInputText(Data);
  }
}
