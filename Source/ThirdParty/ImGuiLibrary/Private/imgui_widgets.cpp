    ...
    if (g.ActiveId == id)
    {
        ...
        if (apply_edit_back_to_user_buffer)
        {
            ...
            if ((flags & (ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_CallbackHistory | ImGuiInputTextFlags_CallbackEdit | ImGuiInputTextFlags_CallbackAlways)) != 0)
            {
                ...
                if ((flags & ImGuiInputTextFlags_CallbackActive) != 0 && set_active_id)
                {
                    event_flag |= ImGuiInputTextFlags_CallbackActive;
                }
                else if ((flags & ImGuiInputTextFlags_CallbackDeactive) != 0 && clear_active_id)
                {
                    event_flag |= ImGuiInputTextFlags_CallbackDeactive;
                }
