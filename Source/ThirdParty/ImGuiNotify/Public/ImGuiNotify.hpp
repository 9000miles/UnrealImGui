...
#define NOTIFY_USE_SEPARATOR \
  true  // If true, a separator will be rendered between the title and the
        // content
#define NOTIFY_USE_DISMISS_BUTTON \
  true  // If true, a dismiss button will be rendered in the top right corner of
        // the toast
#define NOTIFY_RENDER_LIMIT \
  10  // Max number of toasts rendered at the same time. Set to 0 for unlimited
...
class ImGuiToast {
 private:
  ...
  std::function<void()> onButtonPress =
      nullptr;  // A lambda variable, which will be executed when button in
 ...
 public:
   ...
   */
  inline void setType(const ImGuiToastType& intype) {
    IM_ASSERT(intype < ImGuiToastType::COUNT);
    this->type = intype;
  };
   ...
   */
  inline void setWindowFlags(const ImGuiWindowFlags& inflags) {
    this->flags = inflags;
  }
   ...
   *
   * @param onButtonPress std::fuction or lambda expression, which contains the
   * code for execution.
   */
  inline void setOnButtonPress(const std::function<void()>& in_onButtonPress) {
    this->onButtonPress = in_onButtonPress;
  }
 ...
 public:
  ...
  /**
   * @brief Get the elapsed time in milliseconds since the creation of the
   * object.
   *
   * @return int64_t The elapsed time in milliseconds.
   * @throws An exception with the message "Unsupported platform" if the
  ...
  /**
   * @brief Get the current phase of the toast notification based on the elapsed
   ...
   *         - ImGuiToastPhase::Wait: The notification is waiting to be
   * dismissed.
   *         - ImGuiToastPhase::FadeOut: The notification is fading out.
   *         - ImGuiToastPhase::Expired: The notification has expired and should
   * be removed.
   */
  inline ImGuiToastPhase getPhase() {
    const int64_t elapsed =
        std::chrono::duration_cast<std::chrono::milliseconds>(getElapsedTime())
            .count();

    if (elapsed >
        NOTIFY_FADE_IN_OUT_TIME + this->dismissTime + NOTIFY_FADE_IN_OUT_TIME) {
  ...
  /**
   * @return The function, which is executed on the button click in the
   * notification.
   */
 ...
 public:
  ...
  /**
   * @brief Creates a new ImGuiToast object with the specified type and dismiss
   * time.
   *
   * @param type The type of the toast.
   * @param dismissTime The time in milliseconds after which the toast should be
   * dismissed. Default is NOTIFY_DEFAULT_DISMISS.
   */
  ...
  /**
   * @brief Constructor for creating an ImGuiToast object with a specified type
   * and message format.
   *
   * @param type The type of the toast message.
   * @param format The format string for the message.
   * @param ... The variable arguments to be formatted according to the format
   * string.
   */
  ...
  /**
   * @brief Constructor for creating a new ImGuiToast object with a specified
   * type, dismiss time, and content format.
   *
   * @param type The type of the toast message.
   * @param dismissTime The time in milliseconds before the toast message is
   * dismissed.
  ...
  /**
   * @brief Constructor for creating a new ImGuiToast object with a specified
   * type, dismiss time, title format, content format and a button.
   *
   * @param type The type of the toast message.
   * @param dismissTime The time in milliseconds before the toast message is
   * dismissed.
   * @param buttonLabel The label for the button.
   * @param onButtonPress The lambda function to be executed when the button is
   * pressed.
   * @param format The format string for the content of the toast message.
   * @param ... The variable arguments to be formatted according to the format
   * string.
...
inline void SetNotifyViewportSize(const ImVec2& size) { viewport_size = size; }

inline void SetNotifyViewportPos(const ImVec2& pos) { viewport_pos = pos; }

/**
 * Renders all notifications in the notifications vector.
 * Each notification is rendered as a toast window with a title, content and an
 * optional icon. If a notification is expired, it is removed from the vector.
 */
inline void RenderNotifications() {
  const ImVec2 mainWindowSize = viewport_size;
  ...
  for (size_t i = 0; i < notifications.size(); ++i) {
    ...
    const float opacity =
        currentToast
            ->getFadePercent();  // Get opacity based of the current phase
    ...
    // Set notification window position to bottom right corner of the main
    // window, considering the main window size and location in relation to the
    // display
    ImVec2 mainWindowPos = viewport_pos;
    SetNextWindowPos(
    ...
    if (!NOTIFY_USE_DISMISS_BUTTON &&
        currentToast->getOnButtonPress() == nullptr) {
      currentToast->setWindowFlags(NOTIFY_DEFAULT_TOAST_FLAGS |
    ...
    // Here we render the toast content
    {
      PushTextWrapPos(mainWindowSize.x /
                      3.f);  // We want to support multi-line text, this will
                             // wrap the text after 1/3 of the screen width

      bool wasTitleRendered = false;

      const auto dismiss_button_cursor_posy = GetCursorPosY();
      // If an icon is set
      ...
      if (!NOTIFY_NULL_OR_EMPTY(title)) {
        ...
        Text("%s",
             defaultTitle);  // Render default title text
                             // (ImGuiToastType_Success -> "Success", etc...)
        wasTitleRendered = true;
      }


      // In case ANYTHING was rendered in the top, we want to add a small
      // padding so the text (or icon) looks centered vertically
      ...
      if (NOTIFY_USE_DISMISS_BUTTON) {
        // If a title or content is set, we want to render the button on the
        // same line
        if (wasTitleRendered || !NOTIFY_NULL_OR_EMPTY(content)) {
          SetCursorPosY(dismiss_button_cursor_posy - ImGui::GetStyle().FramePadding.y);
          float button_width = ImGui::CalcTextSize(ICON_FA_XMARK).x +
                               ImGui::GetStyle().FramePadding.x * 2;
          const auto dismiss_button_cursor_posx = GetWindowSize().x - GetCurrentWindow()->WindowPadding.x * 2 - button_width;
          SetCursorPosX(dismiss_button_cursor_posx);
          // SameLine(0, ImGui::GetContentRegionAvail().x - button_width);
        ...
        // SetCursorPosX(GetCursorPosX() + (GetWindowSize().x - GetCursorPosX())
        // * scale);
